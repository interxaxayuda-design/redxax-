...
FASE 3 — JUICIO INTEGRADO. Juzgá el hook combinando TODOS los mecanismos de la Fase 2 en conjunto, nunca canal por canal aislado. Un canal débil (ej: plano estático) no vuelve débil al hook si otro canal (ej: el texto) sostiene por sí solo un mecanismo fuerte — evaluá el efecto combinado real, no vetees por un solo canal flojo ignorando los demás. No juzgues por lo que anticipás que viene después, solo por lo que ya está en estos ${hookWindowSegundos} segundos. Formato conocido no es débil por ser conocido. Además: juzgá el mecanismo por su efecto real, nunca por la categoría o industria a la que pertenece el video. Que un contenido pertenezca a un tipo que solés asociar con hooks genéricos (publicidad, corporativo, un nicho específico) no significa que el mecanismo puntual ejecutado acá lo sea — un patrón reconocible (revelación progresiva, escalada, contraste, comparación) puede generar curiosidad genuina si la ejecución específica logra que el espectador necesite ver qué sigue. "Genérico" se responde evaluando si ESTE mecanismo, ejecutado así, produce esa necesidad real — nunca por la etiqueta de categoría del contenido.

FASE 4 — PRUEBA DE FUEGO. Buscá activamente TODAS las razones reales de abandono y TODAS las de permanencia — si hay más de una real, mencionalas todas, no elijas una sola. Contrastá contra los mejores videos que compiten en el feed de ${platform}, no contra videos flojos. Evaluá además: (a) sin producción — cámara mediocre, sin música ni efectos — ¿el mecanismo integrado de la Fase 3 sobrevive?; (b) nicho — la premisa aislada de la producción, ¿presupone un rol o necesidad que la mayoría no tiene?, y la ejecución, ¿es atractiva para cualquiera? Si compiten entre sí, decilo explícitamente en vez de forzar un solo veredicto.
...
FASE 3 — JUICIO INTEGRADO. Juzgá cada escena combinando TODOS los mecanismos de la Fase 2 en conjunto, nunca canal por canal aislado. Un canal débil no vuelve débil a la escena si otro canal sostiene un mecanismo fuerte por sí solo. Evaluá también cómo evoluciona lo planteado en el hook — si avanza, se resuelve, se abandona o se reemplaza. No anticipes escenas futuras al juzgar una escena puntual. Formato conocido no es débil por ser conocido. Además: juzgá el mecanismo por su efecto real, nunca por la categoría o industria a la que pertenece el video. Que un contenido pertenezca a un tipo que solés asociar con hooks genéricos (publicidad, corporativo, un nicho específico) no significa que el mecanismo puntual ejecutado acá lo sea — un patrón reconocible (revelación progresiva, escalada, contraste, comparación) puede generar curiosidad genuina si la ejecución específica logra que el espectador necesite ver qué sigue. "Genérico" se responde evaluando si ESTE mecanismo, ejecutado así, produce esa necesidad real — nunca por la etiqueta de categoría del contenido.
